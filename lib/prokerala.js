...
async function getToken() {
  ...
  if (!r.ok) {
    throw new Error("Prokerala token request failed: " + r.status);
  }
  const j = await r.json();
  _token = j.access_token;
  _exp = now + ((j.expires_in || 3600) * 1000) - 60000; // refresh 1 min early
  return _token;
}

export async function getKundliAdvanced({ datetime, coordinates, ayanamsa = 1, la = "en" }) {
  ...
  const r = await fetch(BASE + "/v2/astrology/kundli/advanced?" + qs.toString(), {
    headers: { Authorization: "Bearer " + token },
  });
  if (!r.ok) {
    const text = await r.text().catch(() => "");
    throw new Error("Prokerala kundli request failed: " + r.status + " " + text.slice(0, 300));
  }
  return r.json();
}
